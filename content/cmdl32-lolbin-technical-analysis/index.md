...
\*There have been a few cases where Microsoft removed potentially dangerous functionality (e.g. `MpCmdRun.exe` downloader, Mandiant's `rasautou.exe` DLL runner, and occasionally built-in UAC bypasses) but these are the exception, not the rule. Also, the potentially dangerous functionality of some built-in programs break the threat model of WDAC and AppLocker, however, these security controls are rarely deployed.
